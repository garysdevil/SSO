...
type User struct {
	...
	Roles     []Role     `json:"roles" gorm:"many2many:t_sso_user_role;JOINTABLE_FOREIGNKEY:user_id;ASSOCIATION_JOINTABLE_FOREIGNKEY:role_id"`
	Menus     []MenuTest `json:"menus" gorm:"-"`
}
...
func (user User) GetUser() (User, []MenuTest, error) {
	...
	//	"b.role_id = c.role_id and a.user_id=?",user.UserId).Scan(&user)
	DB.First(&user)
	var roles []Role
	var menus []MenuTest
